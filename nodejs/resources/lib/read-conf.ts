...
// set file locations
const conf_file: string = './resources/configs/main.conf'
const record_list_file: string = './resources/configs/record_list.conf'
const zones_file: string = './resources/configs/zones.conf'
...
export function getConf() {
    ...
    if (conf != '') {
        ...
        let confArray: string[] = conf.split('\n');

        // remove all comments
        let confNoCommentsArray: string[] = confArray.filter(function (line) {
            return line.indexOf('#') != 0;
        });

        // make the array a sting
        let confNoComments: string = confNoCommentsArray.join('\n');
...
export function getZones() {
    ...
    if (zonesContent != '') {
        // split the sting into an array
        let zonesArray: string[] = zonesContent.split('\n');
...
export function getInterval(confString) {
    // split the sting into an array
    let confArray: string[] = confString.split('\n');
    ...
    if (confIntervalArray[0] == '') // error handling
    {
        // print a error and exit
        ...
        // clean up string
        let confInterval = Number(confIntervalArray[0].replace(/ /g, '').replace(/\t/g, '').replace('interval', '').replace('\r', ''))
...
export function getIPOutput(confString) {
    ...
    if (confIPOutputArray[0] == '') // error handling
    {
        // print a error and exit
        ...
        // clean up string
        let confIPOutput: string = confIPOutputArray[0].replace(/ /g, '').replace(/\t/g, '').replace('ip_output', '').replace('\r', '')
...
export function getUseRecordList(confString) {
    ...
    if (confUseRecordListArray[0] == '') // error handling
    {
        // print a error and exit
